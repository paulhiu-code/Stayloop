/*
  Escape SQL LIKE wildcards in search_properties location filter.
*/
...
BEGIN
  ...
  v_location := public.escape_like_pattern(p_location);
  ...
  WITH filtered AS (
    ...
    WHERE p.is_active = true
      ...
      AND (
        p_location IS NULL
        OR btrim(p_location) = ''
        OR p.title ILIKE '%' || v_location || '%' ESCAPE '\'
        OR p.city ILIKE '%' || v_location || '%' ESCAPE '\'
        OR p.state ILIKE '%' || v_location || '%' ESCAPE '\'
        OR p.country ILIKE '%' || v_location || '%' ESCAPE '\'
        OR p.description ILIKE '%' || v_location || '%' ESCAPE '\'
      )
