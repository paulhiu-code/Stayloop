      ...
      <header className="sticky top-0 z-20 border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-gray-500">{getPhaseName(stepIndex)}</p>
            <h1 className="truncate text-lg font-extrabold text-gray-900 sm:text-xl">{getStepTitle(stepIndex)}</h1>
          </div>
          <button
            type="button"
            onClick={() => void handleSaveAndExit()}
            disabled={saving}
            className="shrink-0 rounded-xl border border-gray-200 px-4 py-2 text-sm font-bold text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {saving ? (
        ...
        {saveError && (
          <p className="mx-auto max-w-3xl px-4 py-2 text-sm text-rose-600 sm:px-6">{saveError}</p>
        )}
      </header>
            ...
            {PROPERTY_TYPE_OPTIONS.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border-2 bg-white p-4 text-left transition hover:border-orange-200 ${
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-bold text-gray-900">{option.label}</p>
                </button>
              );
            })}
          </div>
