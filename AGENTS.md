...
- The existing codebase has pre-existing ESLint errors (mainly `@typescript-eslint/no-explicit-any` in PMS integration code and Supabase edge functions). Lint exits non-zero — this is expected.
- Typecheck (`tsc --noEmit`) also has one pre-existing error (unused import in `Dashboard.tsx`). This is not caused by environment setup.
